// src/main/java/com/appTest/store/controllers/PurchasesReportPdfController.java
...
public class PurchasesReportPdfController {
    ...
    public ResponseEntity<byte[]> purchasesReportPdf(
        ...
        byte[] bytes = reportPdf.renderReport(scope, from, to, supplierId);
        if (bytes == null || bytes.length == 0){
            return ResponseEntity.noContent().build();
        }

        String scopeSlug = scope == null ? "filtered"
