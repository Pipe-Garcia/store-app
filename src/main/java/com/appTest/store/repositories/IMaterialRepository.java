...
public interface IMaterialRepository extends JpaRepository<Material, Long> {
        ...
        @Query("""
        SELECT new com.appTest.store.dto.material.MaterialStockAlertDTO(
            s.material.name,
            s.quantityAvailable
        )
        FROM Stock s
        WHERE s.quantityAvailable < 11
          AND s.material.status = 'ACTIVE'
        """)
        List<MaterialStockAlertDTO> getMaterialsWithLowStock();

        @Query("""
        SELECT new com.appTest.store.dto.material.MaterialMostExpensiveDTO(
            p.name,
            p.priceArs
        )
        FROM Material p
        WHERE p.status = 'ACTIVE'
        ORDER BY p.priceArs DESC
        """)
        List<MaterialMostExpensiveDTO> getMaterialByHighestPrice();

        /**
         * Búsqueda con filtro opcional de familia / precios.
         * - Si includeDeleted = false  → sólo devuelve ACTIVE.
         * - Si includeDeleted = true   → devuelve ACTIVE + INACTIVE.
         */
        @Query("""
        select m
        from Material m
        where (coalesce(:q,'') = '' 
          ...
          and (:includeDeleted = true or m.status = 'ACTIVE')
        order by m.name
        """)
